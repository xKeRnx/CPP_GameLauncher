...
void Patcher::run()
{
	...
	{
		if (!looping) break;
		std::string updateServer = "update" + std::to_string(i) + Constants::UPDATE_DOMAIN;
...
void Patcher::downloadFiles(std::wstring localDirectory, std::string remotePatchDirectory, std::vector<PatchableFile> &remoteFiles, int filesToDownload, int bytesToDownload, int &failedFiles, int &localFaults)
{
	...
	for (int i = 0; i < remoteFiles.size(); i++)
	{
		...
		std::string s_decompressedPath(decompressedPath.begin(), decompressedPath.end());
		...
		if (Downloader::downloadBinaryFile(remoteUrl, compressedPath, &fileDone, &totalBytesDone))
		{
			...
			if (sha == currentFile->sha256)
			{
				...
				fs::path filePath = s_decompressedPath;
				if (filePath.extension() == ".7z")
				{
					try {
						Bit7zLibrary lib{ L"7z.dll" };
						BitExtractor extractor{ lib, BitFormat::SevenZip };

						std::string updateText = "Unpacking:" + currentFile->name;
						Main::extUpdateTopBar(updateText);

						extractor.extract(decompressedPath, localDirectory);
						_wremove(decompressedPath.c_str());
					}
